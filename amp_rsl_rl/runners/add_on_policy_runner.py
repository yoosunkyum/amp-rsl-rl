...
class ADDOnPolicyRunner:
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        # Initilize all the ingredients required for ADD (discriminator, dataset loader)
        num_add_obs = extras["observations"]["add"].shape[1]
        ...
        self.add_normalizer = Normalizer(num_add_obs, device=self.device)
        ...
        alg_class = eval(self.alg_cfg.pop("class_name"))  # ADD_PPO

        self.alg: ADD_PPO = alg_class(
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        obs, extras = self.env.get_observations()
        add_obs = extras["observations"]["add"]
        critic_obs = extras["observations"].get("critic", obs)
        obs, critic_obs = obs.to(self.device), critic_obs.to(self.device)
        add_obs = add_obs.to(self.device)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    self.alg.act_add(add_obs)
                    obs, rewards, dones, infos = self.env.step(actions)
                    _, extras = self.env.get_observations()
                    next_add_obs = extras["observations"]["add"]
                    ...
                    next_add_obs = next_add_obs.to(self.device)

                    # Process the AMP reward
                    style_rewards = self.discriminator.predict_reward_add(
                        add_obs, normalizer=self.add_normalizer)
                    ...
                    self.alg.process_add_step(next_add_obs)

                    # The next observation becomes the current observation for the next step
                    add_obs = torch.clone(next_add_obs)
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        self.writer.add_scalar("Loss/add_loss", locs["mean_add_loss"], locs["it"])
        self.writer.add_scalar(
    ...
    def save(self, path, infos=None, save_onnx=False):
        saved_dict = {
            ...
            "add_normalizer": self.alg.add_normalizer,
    ...
    def load(self, path, load_optimizer=True, weights_only=False):
        ...
        self.alg.add_normalizer = loaded_dict["add_normalizer"]
